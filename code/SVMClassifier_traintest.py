...
# file_list = ['en', 'it', 'es']
# train_lang = ['it', 'es', 'pt', 'fr', 'de']

test_events = ['LAX', 'BOB', 'WTX']

# test_events = ['TPY', 'TYP', 'ALB', 'QFL', 'CFL', 'PHF', 'SAR']

train_lang = ['it', 'es', 'pt', 'fr', 'de', 'en']
# train_lang = ['es']

file_list = ['it', 'es', 'pt', 'fr', 'de', 'en']

for event_t in test_events:

    for train_lg in train_lang:
        ...
        # train_file = 'balanced_random_' + f_lst_temp + '_stat.csv'

        # train_file = 'balanced_random_' + f_lst_temp + '_stat_annotationlemma_hypernym_semantics.csv'

        train_file = 'balanced_random_' + f_lst_temp + '_stat_dbpedia_en_semantics.csv'
        ...
        with open('/Users/pk4634/Documents/phase3/iswc_eswc/' + folder + '/' + train_file) as fline:
        ...
        # t = ['tweetID', 'Document', 'infotype', 'relatedness', 'lang', 'event',
        #     'NumberOfNouns', 'NumberOfVerbs', 'NumberOfPronouns', 'TweetLength', 'NumberOfWords', 'NumberOfHashTag', 'related_score']

        t = ['tweetID', 'Document', 'event',
             'NumberOfNouns', 'NumberOfVerbs', 'NumberOfPronouns', 'TweetLength', 'NumberOfWords', 'NumberOfHashTag', 'related_score']

        wholedata_frame = pd.DataFrame(data, columns=t)

        frm = wholedata_frame.loc[wholedata_frame['event'] != 'LAX']

        frm = frm.loc[frm['event'] != 'BOB']

        frm = frm.loc[frm['event'] != 'WTX']

        # frm = frm.loc[frm['event'] != 'COS']

        # frm = frm.loc[frm['event'] != 'CFL']

        # frm = frm.loc[frm['event'] != 'PHF']

        # frm = frm.loc[frm['event'] != 'SAR']

        # frm = frm.loc[frm['event'] != 'MNL']

        Y = frm.related_score.astype(np.float32)
        ...
        for f_lst in file_list:
            ...
            # test_file = 'balanced_random_' + f_lst + '_stat.csv'

            # test_file = 'balanced_random_' + f_lst + '_stat_annotationlemma_hypernym_semantics.csv'

            test_file = 'balanced_random_' + f_lst + '_stat_dbpedia_en_semantics.csv'
            ...
            print 'test_event is ' + test_event
            print 'train lang is' + ' ' + f_lst_temp
            print 'test lang is' + ' ' + f_lst
            ...
            with open('/Users/pk4634/Documents/phase3/iswc_eswc/' + folder + '/' + test_file) as fline2:
                ...
                # t_test = ['tweetID', 'Document', 'infotype', 'relatedness', 'lang', 'event',
                #          'NumberOfNouns', 'NumberOfVerbs', 'NumberOfPronouns', 'TweetLength', 'NumberOfWords', 'NumberOfHashTag', 'related_score']

                t_test = ['tweetID', 'Document', 'event',
                          'NumberOfNouns', 'NumberOfVerbs', 'NumberOfPronouns', 'TweetLength', 'NumberOfWords', 'NumberOfHashTag', 'related_score']

                wholedata_frame_test = pd.DataFrame(data_test, columns=t_test)

                frm_test = wholedata_frame_test.loc[wholedata_frame_test['event'] == test_event]

                # frm_test = frm_test.loc[frm_test['event'] != 'BOB']

                # frm_test = frm_test.loc[frm_test['event'] != 'WTX']

                Y_test = frm_test.related_score.astype(np.float32)
                ...
                #print('fscore: {}'.format(f))
                print('')
                print(precision_score(Y_test, Y_test_predict.astype(np.float32), average='macro'))
                print(recall_score(Y_test, Y_test_predict.astype(np.float32), average='macro'))
                print(f1_score(Y_test, Y_test_predict.astype(np.float32), average='macro'))
                print('')
                print(confusion_matrix(Y_test, Y_test_predict.astype(np.float32)))
                print('')
                ...
                # Y_test, Y_test_predict.astype(np.float32))))
